...
export default function CompleteProfileCard() {
  ...
  const { uid, displayName, email, photoURL, profileCompleted } = useSelector((state) => state.auth);
  ...
  const handleSaveProfile = async (userData) => {
    try {
      ...
      if (result.ok) {
        ...
        await dispatch(startUpdateProfileCompleted({ uid, profileCompleted: true }));
